...
var CustomImportScript = (() => {
  ...
  function parse2(element, { document: document2 }) {
    ...
    const videoUrl = iframe ? iframe.getAttribute("data-media-src") || iframe.getAttribute("src") || iframe.getAttribute("data-src") : null;
    const posterUrl = iframe ? iframe.getAttribute("data-src") || iframe.getAttribute("data-poster") : null;
    ...
    const cells = [];
    const uriCell = document2.createDocumentFragment();
    if (videoUrl) {
      uriCell.appendChild(document2.createComment(" field:uri "));
    ...
    const posterCell = document2.createDocumentFragment();
    if (posterUrl) {
      const img = document2.createElement("img");
      img.setAttribute("src", posterUrl);
      img.setAttribute("alt", titleText);
      posterCell.appendChild(document2.createComment(" field:placeholder_image "));
  ...
  // tools/importer/parsers/card-carousel.js
  function parse4(element, { document: document2 }) {
    ...
    const cells = [];
    const items = element.classList.contains("grid-item") ? [element] : [...element.querySelectorAll(".grid-item")];
    items.forEach((item) => {
      ...
      const videoHref = headingLink ? headingLink.getAttribute("href") : (primary.querySelector("a.article-link") || {}).getAttribute ? primary.querySelector("a.article-link").getAttribute("href") : null;
      const textNodes = [];
      if (tag) {
        const p = document2.createElement("p");
    ...
    const block = WebImporter.Blocks.createBlock(document2, { name: "card-carousel", cells });
    element.replaceWith(block);
  }
  ...
  function parse6(element, { document: document2 }) {
    const promoteImg = (img) => {
      if (!img) return null;
      const real = img.getAttribute("data-flickity-lazyload") || img.getAttribute("data-src") || img.getAttribute("data-src-lg");
      if (real && (!img.getAttribute("src") || /BackgroundGradLoad|blank|placeholder/i.test(img.getAttribute("src") || ""))) {
        img.setAttribute("src", real);
      }
    ...
    const cells = [];
    const slides = element.classList.contains("caro-item") ? [element] : [...element.querySelectorAll(".caro-item")];
    slides.forEach((slide) => {
      const link = slide.querySelector("a[href]");
      const img = promoteImg(slide.querySelector("img"));
      const href = link ? link.getAttribute("href") : null;
      const label = link ? (link.getAttribute("aria-label") || img && img.getAttribute("alt") || "").trim() : (img && img.getAttribute("alt") || "").trim();
      if (!img && !href) return;
      const imageCell = document2.createDocumentFragment();
      if (img) {
        imageCell.appendChild(document2.createComment(" field:backgroundImage "));
        imageCell.appendChild(img);
      }
  ...
  function parse7(element, { document: document2 }) {
    ...
    const action = submit && submit.getAttribute("href") && submit.getAttribute("href") !== "#" ? submit.getAttribute("href") : null;
    const cells = [];
    if (action) {
      const actionCell = document2.createDocumentFragment();
      actionCell.appendChild(document2.createComment(" field:action "));
      actionCell.appendChild(document2.createTextNode(action));
